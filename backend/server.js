...
app.post('/api/assignments', async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Error saving assignment', error: error.message });
  }
});
...
app.delete('/api/assignments', async (req, res) => {
  try {
    ...
    if (assignments.length === 0) {
      return res.status(404).json({ message: 'No assignments found for this person on the given date' });
    }

    // Delete the specified assignment (first or second)
    const assignmentToDelete = assignments[assignmentNumber - 1];
    if (!assignmentToDelete) {
      return res.status(404).json({ message: 'Specified assignment not found' });
    }
...
async function generateDailyReport(date, employee) {
  ...
  console.log(`Generating report for date: ${startDate.toISOString()} to ${endDate.toISOString()}`);
  if (employee) {
  ...
  let messages = await mongoose.connection.db.collection('mqttMsg').find(query).toArray();
  ...
  console.log(`Report results - Good Valves: ${goodValves}, Invalid Valves: ${invalidValves}`);
