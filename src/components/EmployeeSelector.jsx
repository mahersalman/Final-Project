...
const EmployeeSelector = ({ selectedEmployees, onChange, maxSelections, selectedStation }) => {
  ...
  useEffect(() => {
    const fetchSortedEmployees = async () => {
      if (selectedStation) {
        ...
        setError(null);
        try {
          const response = await axios.get(`http://localhost:5001/api/sorted-employees/${selectedStation.station_name}`);
  ...
  const handleCheckboxChange = (employee) => {
    const isSelected = selectedEmployees.some(e => e._doc._id === employee._doc._id);
    let updatedEmployees;
    if (isSelected) {
      updatedEmployees = selectedEmployees.filter(e => e._doc._id !== employee._doc._id);
        ...
        {isLoading ? (
          ...
          employeeOptions.length > 0 ? (
            employeeOptions.map((employee, index) => (
              <div key={employee._doc._id} className="flex items-center justify-between p-3 border-b border-gray-200 last:border-b-0 hover:bg-gray-50 transition-colors duration-150">
                <div className="flex items-center">
                  <input
                    type="checkbox"
                    id={`employee-${employee._doc._id}`}
                    checked={selectedEmployees.some(e => e._doc._id === employee._doc._id)}
                    onChange={() => handleCheckboxChange(employee)}
                    disabled={!selectedEmployees.some(e => e._doc._id === employee._doc._id) && selectedEmployees.length >= maxSelections}
                  ...
                  />
                  <label htmlFor={`employee-${employee._doc._id}`} className="mr-3 text-sm font-medium text-gray-700">
